...
| **A repair-procedure skill in this repo** | Built as `/media-audit` on 1 September 2026 and retired the same day. `site-runbooks` already held the same three phases and the same finding explanations, vendor-neutral and better written — and ⚠ **both skills triggered on nearly identical prompts**, so which one answered was arbitrary. The division that holds: an executable check lives here where `test:gates` proves it still refuses; a repair procedure is long-form judgement that cannot be tested and belongs in the runbooks |
